                                     ...
                                     float vel_ff, float torque_ff)
{
    ...
    odrive_pack_i16(&p[4], clamp_i16((vel_ff / od->motor_conv) * 1000.0f));
    /* torque_ff: Nm (no conversion) -> raw int16 (*1000) */
    odrive_pack_i16(&p[6], clamp_i16(torque_ff * 1000.0f));
    return odrive_send_frame(od, ODRIVE_CMD_SET_INPUT_POS, p, 8, false);
}
...
odrive_status_t odrive_set_input_vel(odrive_t *od, float vel, float torque_ff)
{
    ...
    return odrive_send_frame(od, ODRIVE_CMD_SET_INPUT_VEL, p, 8, false);
}
...
odrive_status_t odrive_set_input_torque(odrive_t *od, float torque)
{
    ...
    return odrive_send_frame(od, ODRIVE_CMD_SET_INPUT_TORQUE, p, 4, false);
}
...
odrive_status_t odrive_set_absolute_position(odrive_t *od, float pos)
{
    ...
    uint8_t p[4] = {0};
    odrive_pack_f32(&p[0], pos / od->motor_conv);
    return odrive_send_frame(od, ODRIVE_CMD_SET_ABSOLUTE_POSITION, p, 4, false);
}
